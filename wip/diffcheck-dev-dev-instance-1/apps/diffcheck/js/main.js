...
function renderDiff(changes) {
  ...
  changes.forEach(change => {
    ...
    lineNumDiv.textContent = change.oldLineNum ?? change.newLineNum ?? '';
    
    const contentDiv = document.createElement('div');
    contentDiv.className = 'diff-line-content';
    contentDiv.textContent = `${prefix} ${change.line}`;
  ...
  diffStats.textContent = `${additions} addition(s), ${deletions} deletion(s)`;
  diffOutput.hidden = false;
}
