...
## Contract pin — POST /rank (FROZEN, cycle 25 verification)

Seam gate ① resolved: CTO verified the shipped M2 implementation (read-only) — the weights-override schema is FROZEN. See `/data/debates/m3-suggestion-api-webui.md` §"Seam-gate resolution" for the full determination. M3-B MUST code against exactly this:

- **Request shape:** `POST /rank {symbols[], as_of_date, algorithm_version, weights?, series{}}`.
- **`weights` optional; if present:** key set must be exactly `{momentum, trend, volume, volatility}` (missing OR extra keys → `400 INVALID_INPUT`); each value a float in `[0,1]`; sum must equal 1.0 within ±0.001 → else `400 INVALID_INPUT`.
- **Response:** effective weights echoed as `weights_used`.
- **Range [0,1] is NOT enforced server-side** — M3-B must send in-range weights (UC-SA-3 precondition).
- **Caveats:** positive full-override path untested (QA add-on at M3 gate); UC-AE-3 doc drift — use `as_of_date` not `date`, and `series` map is required (shipped code is authoritative).
- **Fallback trigger does NOT fire** — task 16 builds against the frozen contract; no narrower scope needed.
